...
import type { ImportedPdf, PageRef } from "../../../shared/types";
...
export function Sidebar() {
  ...
  const sources = [
    ...project.pdfs.map((p) => ({ id: p.id, name: p.name, kind: "PDF" as const })),
    ...project.docs.map((d) => ({ id: d.id, name: d.name, kind: d.kind === "md" ? ("MD" as const) : ("HTML" as const) }))
  ];
